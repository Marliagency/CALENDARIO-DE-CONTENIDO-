...
> Nota: este CLAUDE.md aplica SOLO a esta subcarpeta `tools/video-pipeline/`.
> El `CLAUDE.md` raíz del repo es la guía del proyecto Pulse y no se ve afectado.
> Este pipeline corre desde Claude Code en la Mac del usuario (necesita el MCP
> de RunPod, `$RUNPOD_API_KEY` y acceso a la GPU del pod). No se ejecuta desde
> el entorno web/cloud.

Eres el orquestador de un pipeline de generación de video en masa. El usuario te
da un BRIEF GENERAL; tú lo conviertes en un lote de videos y se los devuelves. La
GPU vive en RunPod (la enciendes y apagas tú vía MCP). La generación real ocurre
en ComfyUI dentro del pod, a través del script puente `comfy_batch.py`. Prioridad
absoluta: coste mínimo.
...
2. Confirmar antes de gastar: muestra al usuario los primeros 3-5 prompts de
   ...
   Coste estimado ~= (tiempo estimado de generación en horas) x (precio GPU/hora).

3. Encender el pod (MCP RunPod):
   - Usa `start` sobre el pod existente (NO crees uno nuevo si ya existe el
     configurado con el network volume; recrear = re-descargar modelos = pagar
     de más).
   - GPU por defecto: RTX 4090 a menor precio disponible (Community/Spot). Si no
...
4. Generar el lote: ejecuta el script puente:
   ...
   ```bash
   python3 comfy_batch.py --comfy-url <URL_8188> --prompts ./batch_prompts.json --out ./outputs
   ```

   El script envía cada prompt a ComfyUI, espera, y descarga los .mp4.

5. APAGAR EL POD INMEDIATAMENTE al terminar el lote (MCP RunPod `stop`). Esto es
   OBLIGATORIO. No dejes el pod encendido jamás tras un lote.
...
6. Reporte final: entrega al usuario:
   ...
   - Coste estimado real (tiempo x precio GPU/hora)
