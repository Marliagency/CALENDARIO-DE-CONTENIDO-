...
"$FF" -y \
  ...
  -f lavfi -i "aevalsrc=sin(2*PI*80*t)*exp(-22*(t-floor(t*1.9666)*0.5085))*0.52+sin(2*PI*42*t)*exp(-28*(t-floor(t*1.9666)*0.5085))*0.36:s=44100:d=55" \
  -f lavfi -i "aevalsrc=(2*random(0)-1)*exp(-55*(t-floor(t*3.934)*0.2542))*0.18:s=44100:d=55" \
  -filter_complex \
